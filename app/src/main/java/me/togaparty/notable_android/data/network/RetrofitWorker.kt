...
class RetrofitWorker(val context: Context) {
    ...
    fun uploadFile(currentImage: GalleryImage): GalleryImage {
        ...
        val response: Response<ResponseBody>? = try {
            ...
            throw IOException("Failed to upload the file to the server. Please contact support for more inquiries.")
        } catch (exec: RuntimeException) {
            exec.printStackTrace()
            throw RuntimeException("Failed to decode the response from the server.Please contact support for more inquiries.")
        }
        ...
        if (response != null) {
            if (response.isSuccessful) {
                ...
                throw IOException("Upload failed. Server might be down. Please contact support for more inquiries")
            }
        }
